...
private:
  ...
  static shared_ptr<Node> Build(IndexSegment segment) {
    if (segment.empty()) {
      return nullptr;
  ...
  static typename Visitor::ResultType TraverseWithQuery(shared_ptr<Node> node, IndexSegment query_segment, Visitor visitor) {
  ...
  public:
    ...
    Data ProcessEmpty(shared_ptr<Node>) const {
      return {};
    }

    Data ProcessFull(shared_ptr<Node> node) const {
      return node->data;
    }

    Data ProcessPartial(shared_ptr<Node>, IndexSegment, const Data& left_result, const Data& right_result) const {
  ...
  public:
    ...
    void ProcessEmpty(shared_ptr<Node>) const {}

    void ProcessFull(shared_ptr<Node> node) const {
      node->postponed_bulk_operation.CombineWith(operation_);
      node->data = operation_.Collapse(node->data, node->segment);
    }

    void ProcessPartial(shared_ptr<Node> node, IndexSegment) const {
  ...
  static void PropagateBulkOperation(shared_ptr<Node> node) {
...
struct AddMoneyRequest : ModifyRequest {
  ...
  void Process(BudgetManager& manager) const override {
    ...
    const MoneyState daily_change = SIGN == 1 ? MoneyState{.earned = daily_value, .spent = 0.0} : MoneyState{.earned = 0.0, .spent = daily_value};
...
void RunTest(){
    ...
    ss << "PayTax 2000-01-02 2000-01-03 13\n";
    ss << "ComputeIncome 2000-01-01 2001-01-01\n";
    ss << "Spend 2000-12-30 2001-01-02 14\n";
    ss << "ComputeIncome 2000-01-01 2001-01-01\n";
    ss << "PayTax 2000-12-30 2000-12-30 13\n";
    ss << "ComputeIncome 2000-01-01 2001-01-01\n";
